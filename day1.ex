defmodule DayOne do
  ...
  def turn_str_into_num_seq(str) do
    ...
    word_indices = Enum.map(numbers, &({&1, find_indices_of_substring(str, &1)})) |> Enum.flat_map(&spread.(&1)) |> Enum.map(&({num_value_map[elem(&1,0)], elem(&1,1)}))
    ...
    sorted = Enum.sort(number_indices, &(select.(&1) < select.(&2))) |> Enum.filter(filter_condition)
    final = sorted
      |> Enum.map(&elem(&1,0))
      |> Enum.join()
